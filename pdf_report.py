...
class NumberedCanvas(rl_canvas.Canvas):
    ...
    def _stamp(self, pnum, total):
        ...
        self.drawString(MARGIN, H - 0.73*cm, "BusinessNext  AWS Cost Estimator  ·  Pricing Report")
...
def generate_pdf_report(
    ...
    doc = BaseDocTemplate(
        ...
        title=f"AWS Pricing Report — {customer}",
    ...
    story.append(SP(10))
    story.append(P("AWS Cost Estimator", S_TITLE))
    story.append(SP(4))
    ...
    notes = [
        ...
        ("Fallback Rates",
         ...
         "as estimated."),
        ("Node Distribution",
         "Infrastructure sizing computed by a rule-based algorithm, optionally enhanced by "
         "Groq LLM (llama-3.3-70b-versatile) at temperature=0. SHA-256 caching prevents "
         "redundant API calls."),
        ("Named & Concurrent Users",
         "Concurrent users are fixed at 30% of Total Named Users for every forecast year. "
         "Year-over-Year growth is applied per the rates selected in the estimator tool."),
        ...
        ("GCP Pricing",
         "GCP prices are hardcoded us-central1 On-Demand Compute Engine estimates (March 2026). "
         "GCP Committed Use Discounts (1-Year CUD) typically reduce costs by ~37%."),
        ("PUPM Formula",
         "PUPM = Discounted Annual Cost / 12 / Named Users. Discounted cost = Total AWS Cost "
         "x (1 - 11% discount). Total includes production, DR, Pre-Prod, buffer (5%), "
         "Business Support (3.91%), and one-time migration costs in Year 1."),
        ("Managed Services",
    ...
    story.append(P(
        f"Generated by BusinessNext AWS Cost Estimator  ·  {today}  ·  Internal Use Only  ·  "
