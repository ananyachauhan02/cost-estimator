...
def generate_pdf_report(
    ...
    # Resolve AI sizing — prefer explicit param, fall back to distribution
    _ai = ai_sizing or (dist.get("ai_nodes") if dist else None) or {}
    ...
    story += section_header("Executive Summary",
                            "Consolidated cost overview — all environments included")
    story.append(SP(6))

    if client_mode == "saas" and pricing:
        prod_monthly = pricing.get("total_monthly_usd", 0)
        infr         = pricing.get("inflation_rate", 0.04)
        named_u      = int(metrics.get("total_named_users", 0) or 0)

        # ── Compute Grand Total (Prod + Pre-Prod/SIT/UAT + DR + AI) ────────────
        pp_monthly  = float((env_pricing or {}).get("preprod_sit_uat", {}).get("monthly_usd", 0) or 0) \
                      if env_pricing and (env_pricing or {}).get("preprod_sit_uat") else 0.0
        dr_monthly  = float((env_pricing or {}).get("dr", {}).get("monthly_usd", 0) or 0) \
                      if env_pricing and (env_pricing or {}).get("dr") else 0.0
        
        ai_mo = 0.0
        _ai = ai_sizing or {}
        if _ai.get("enabled"):
            cs = _ai.get("combined_summary", {})
            ai_mo = round(cs.get("total_worker_nodes", 0) * 0.576 * 730 + cs.get("total_storage_gb", 0) * 0.08 + cs.get("bedrock_monthly", 3000), 2)
            
        grand_monthly = round(prod_monthly + pp_monthly + dr_monthly + ai_mo, 2)
        grand_annual  = round(grand_monthly * 12, 2)
        # 5-year grand total: inflate each year at inflation_rate
        grand_5yr = round(sum(grand_monthly * 12 * ((1 + infr) ** y) for y in range(1, 6)), 2)

        story.append(kpi_strip([
            ("Grand Total Monthly",  fmt(grand_monthly), "Prod + Pre-Prod + DR",         AZURE),
            ("Grand Total Annual",   fmt(grand_annual),  "All environments · Year 1",    VIOLET),
            ("5-Year TCO",           fmt(grand_5yr),     f"At {infr*100:.0f}% inflation/yr", TEAL),
            ("Named Users",          f"{named_u:,}" if named_u else "—",
                                     "Year 1 licensed",                                  GOLD),
        ]))
        story.append(SP(8))

        # Hero cost band — Grand Total
        story.append(cost_band("", grand_monthly, grand_annual, grand_5yr, AZURE))

        story.append(SP(6))
        story.append(P(
            "<b>Note:</b> The figures above represent the complete all-in infrastructure cost "
            "including Production, all selected non-production environments (Pre-Prod / SIT / UAT) "
            "and the Disaster Recovery environment. A full per-environment breakdown follows on the next page.",
            sty("exec_note", fontSize=8, textColor=GRAY5, leading=12, spaceBefore=4)
        ))
    ...
    story.append(info_card([
        ...
        ("Concurrent Users",      f"{int(metrics.get('concurrent_users', 0) or 0):,}"),
    ...
    # ══════════════════════════════════════════════════════════════════════
    # SECTION 1b — CONSOLIDATED COST SUMMARY (new dedicated page)
    # ══════════════════════════════════════════════════════════════════════
    if client_mode == "saas" and pricing:
        ...
        story += section_header(
            ...
            "All-environment monthly and annual cost breakdown",
            color=GREEN_DARK,
        )
        story.append(SP(8))

        # ── Re-derive numbers (prod_monthly etc. may not be in scope) ────
        _prod_mo    = pricing.get("total_monthly_usd", 0)
        _prod_5yr   = pricing.get("inflation_forecast", {}).get("five_year_total", 0)
        _infr       = pricing.get("inflation_rate", 0.04)
        _pp         = (env_pricing or {}).get("preprod_sit_uat") or {}
        _dr         = (env_pricing or {}).get("dr") or {}
        _pp_mo      = float(_pp.get("monthly_usd", 0) or 0)
        _dr_mo      = float(_dr.get("monthly_usd", 0) or 0)
        _ai_mo = 0.0
        _ai = ai_sizing or {}
        if _ai.get("enabled"):
            _cs = _ai.get("combined_summary", {})
            _ai_mo = round(_cs.get("total_worker_nodes", 0) * 0.576 * 730 + _cs.get("total_storage_gb", 0) * 0.08 + _cs.get("bedrock_monthly", 3000), 2)

        _grand_mo   = round(_prod_mo + _pp_mo + _dr_mo + _ai_mo, 2)
        _grand_yr   = round(_grand_mo * 12, 2)
        _grand_5yr  = round(sum(_grand_mo * 12 * ((1 + _infr) ** y) for y in range(1, 6)), 2)
        _pp_5yr     = round(sum(_pp_mo * 12 * ((1 + _infr) ** y) for y in range(1, 6)), 2) if _pp_mo else 0
        _dr_5yr     = _dr.get("five_year_forecast", {}).get("five_year_total", 0) or 0
        _ai_5yr     = round(sum(_ai_mo * 12 * ((1 + _infr) ** y) for y in range(1, 6)), 2) if _ai_mo else 0

        # ── Environment rows for the summary table ────────────────────────
        _pp_names = _pp.get("env_names", [])
        _pp_label = ", ".join(_pp_names) if _pp_names else "Pre-Prod / SIT / UAT"
        _dr_scale = (env_pricing or {}).get("dr_scale", 1.0)
        _dr_label = f"Disaster Recovery  ({int(_dr_scale * 100)}% scale)"
        ...
        if _pp and _pp_mo > 0:
            _pp_mult = _pp.get("env_multiplier", 1) or 1
            _env_rows.append([
                f"🔵  {_pp_label}",
                fmt(_pp_mo),
                fmt(_pp_mo * 12),
                fmt(_pp_5yr),
                f"{_pp_mult} env(s) × 40% of Prod",
            ])
        ...
        if _dr and _dr_mo > 0:
            _env_rows.append([
                ...
                fmt(_dr_mo * 12),
                fmt(_dr_5yr),
                f"Compute {int(_dr_scale * 100)}% · DB 100% of Prod",
            ])
        ...
        _col_widths = [4.2*cm, 2.8*cm, 3.2*cm, 3.4*cm, 5.2*cm]
        _tdata = [[P(h, S_TH) for h in ["Environment", "Monthly", "Annual (Yr 1)", "5-Year TCO", "Sizing Notes"]]]
        for _i, _row in enumerate(_env_rows):
            _is_total = (_i == len(_env_rows) - 1)
            _style = sty(
                f"envr{_i}",
                fontName="Helvetica-Bold" if _is_total else "Helvetica",
                fontSize=8.5 if _is_total else 8,
                textColor=GREEN_DARK if _is_total else DARK,
                alignment=TA_LEFT,
                leading=11,
            )
            _money_style = sty(
                ...
                fontName="Helvetica-Bold" if _is_total else "Helvetica",
                fontSize=8.5 if _is_total else 8,
                textColor=GREEN_MID if _is_total else DARK,
                alignment=TA_RIGHT,
                leading=11,
            )
            _tdata.append([
                P(_row[0], _style),
                P(_row[1], _money_style),
                P(_row[2], _money_style),
                P(_row[3], _money_style),
                P(_row[4], sty(f"envcmt{_i}", fontSize=7.5, textColor=GRAY5 if not _is_total else GREEN_DARK, leading=10)),
            ])

        _ts = TableStyle([
            ("BACKGROUND",    (0, 0), (-1, 0),  NAVY),
            ("ROWBACKGROUNDS",(0, 1), (-1, -2), [WHITE, GREEN_BG]),
            ("BACKGROUND",    (0, -1),(-1, -1), colors.HexColor("#ECFDF5")),
            ("LINEABOVE",     (0, 0), (-1,  0),  2.5, GREEN_DARK),
            ("LINEABOVE",     (0, -1),(-1, -1),  1.5, GREEN_MID),
            ("LINEBELOW",     (0, -1),(-1, -1),  1.5, GREEN_MID),
            ("GRID",          (0, 0), (-1, -1),  0.3, GRAY3),
            ("TOPPADDING",    (0, 0), (-1, -1),  6),
            ("BOTTOMPADDING", (0, 0), (-1, -1),  6),
        ...
        story.append(Table(_tdata, colWidths=_col_widths, style=_ts, repeatRows=1))
        story.append(SP(12))

        # ── Per-environment monthly+annual detail cards ───────────────────
        story.append(P("Environment Cost Detail", S_H2))
        story.append(SP(4))

        # Production detail
        story.append(info_card([
            ("Production — Monthly",      fmt(_prod_mo)),
            ("Production — Annual",       fmt(_prod_mo * 12)),
            ("Production — 5-Year TCO",   fmt(_prod_5yr)),
            ("Deployment",                "Multi-AZ High Availability · EKS · Patroni HA PostgreSQL"),
            ("Region",                    pricing.get("region_label", pricing.get("region", "—"))),
        ], fill=AZURE_BG, accent=AZURE))
        story.append(SP(8))

        # Pre-Prod / SIT / UAT detail
        if _pp and _pp_mo > 0:
            _pp_mult = _pp.get("env_multiplier", 1) or 1
            _base_pp = round(_pp_mo / _pp_mult, 2) if _pp_mult > 1 else _pp_mo
            story.append(info_card([
                ("Environments",              f"{_pp_mult} — {_pp_label}"),
                ("Cost per Environment / mo", fmt(_base_pp)),
                ("Cost per Environment / yr", fmt(_base_pp * 12)),
                ("Total Pre-Prod Monthly",    fmt(_pp_mo)),
                ("Total Pre-Prod Annual",     fmt(_pp_mo * 12)),
                ("Total Pre-Prod 5-Year",     fmt(_pp_5yr)),
                ("Sizing",                    "40% of Production compute · Single-AZ"),
        ...
        if _dr and _dr_mo > 0:
            story.append(info_card([
                ("DR — Monthly",              fmt(_dr_mo)),
                ("DR — Annual",               fmt(_dr_mo * 12)),
                ("DR — 5-Year TCO",           fmt(_dr_5yr)),
                ("Compute Scale",             f"{int(_dr_scale * 100)}% of Production worker nodes"),
                ("DB Scale",                  "100% of Production — full replication always active"),
                ("Deployment",                "Multi-AZ · Warm-Standby" if _dr_scale < 1.0 else "Multi-AZ · Full Mirror"),
            ], fill=RED_BG, accent=RED))
            story.append(SP(8))

        # Grand total summary card
        story.append(info_card([
            ("GRAND TOTAL — Monthly",      fmt(_grand_mo)),
            ("GRAND TOTAL — Annual",       fmt(_grand_yr)),
            ("GRAND TOTAL — 5-Year TCO",   fmt(_grand_5yr)),
            ("Average Monthly (5-yr adj.)", fmt(round(_grand_5yr / 60, 2))),
            ("Inflation Rate Applied",      f"{_infr * 100:.0f}% per year from Year 2"),
        ], fill=GREEN_BG, accent=GREEN_DARK))
    ...
    ch_sizing = dist.get("clickhouse_nodes", {})
    ch_enabled = ch_sizing and ch_sizing.get("enabled")
    ch_total   = ch_sizing.get("summary", {}).get("total_nodes", 0) if ch_enabled else 0
    ...
    story.append(info_card([
        ...
        ("ClickHouse OLAP Nodes", ch_total if ch_enabled else "Not enabled"),
    ...
    # ══════════════════════════════════════════════════════════════════════
    # SECTION 2b — CLICKHOUSE OLAP CLUSTER (when enabled)
    # ══════════════════════════════════════════════════════════════════════
    if ch_enabled:
        ORANGE = colors.HexColor("#EA580C")
        ORANGE_BG = colors.HexColor("#FFF7ED")
        ORANGE_BG2 = colors.HexColor("#FFEDD5")

        story += section_header(
            "ClickHouse OLAP Cluster",
            "Self-hosted sharded/replicated ClickHouse DB + Keeper coordination cluster",
            color=ORANGE,
        )
        story.append(SP(6))

        ch_db  = ch_sizing.get("db_cluster",     {})
        ch_kp  = ch_sizing.get("keeper_cluster", {})
        ch_sum = ch_sizing.get("summary",        {})

        # Summary card
        story.append(info_card([
            ("Analytics Data Volume",
             f"{ch_sizing.get('ch_data_gb', 0):,.0f} GB  "
             f"({ch_sizing.get('ch_data_multiplier', 2.0)}× transactional DB × "
             f"{ch_sizing.get('volume_factor', 1.0)}× volume factor)"),
            ("Total Cluster Nodes",  f"{ch_sum.get('total_nodes', 0)}  "
             f"({ch_db.get('total_nodes', 0)} DB + {ch_kp.get('total_nodes', 0)} Keeper)"),
            ("DB Shards × Replicas",
             f"{ch_db.get('num_shards', 1)} shard(s) × {ch_db.get('replicas_per_shard', 2)} replica(s)"),
            ("Total vCPU",  str(ch_sum.get("total_vcpu",  0))),
            ("Total RAM",   f"{ch_sum.get('total_ram_gb', 0)} GB"),
            ("Total Storage", f"{ch_sum.get('total_storage_gb', 0):,} GB (EBS gp3)"),
            ("DB Node Spec",
             f"{ch_db.get('vcpu_per_node', 0)} vCPU / {ch_db.get('ram_per_node', 0)} GB RAM / "
             f"{ch_db.get('storage_per_node_gb', 0):,} GB SSD each"),
            ("Keeper Node Spec",
             f"{ch_kp.get('vcpu_per_node', 0)} vCPU / {ch_kp.get('ram_per_node', 0)} GB RAM / "
             f"{ch_kp.get('storage_per_node_gb', 0)} GB SSD each"),
            ("Hosting",     "Self-hosted on EC2  —  same for SaaS and On-Prem"),
        ], fill=ORANGE_BG, accent=ORANGE))
        story.append(SP(10))

        # Table 1: DB cluster
        story.append(P("ClickHouse DB Cluster — Sharded + Replicated", S_H2))
        db_rows = []
        n_shards = ch_db.get("num_shards", 1) or 1
        shard_ram  = ch_db.get("total_ram_gb",  0) // n_shards
        shard_vcpu = ch_db.get("total_vcpu",     0) // n_shards
        for node in ch_db.get("nodes", []):
            db_rows.append([
                f"Shard {node['shard']}",
                f"Replica {node['replica']}",
                str(node["vcpu_per_node"]),
                str(node["ram_per_node"]),
                str(shard_ram),
                str(shard_vcpu),
                f"{node['storage_per_node_gb']:,} GB",
            ])
        if db_rows:
            story.append(data_table(
                ["Shard", "Node", "vCPU", "RAM (GB)",
                 "Total RAM/Shard (GB)", "Total Cores/Shard", "Storage/Node"],
                db_rows,
                col_widths=[1.8*cm, 2.0*cm, 1.6*cm, 2.2*cm, 3.8*cm, 3.4*cm, 2.0*cm],
                accent=ORANGE, alt=ORANGE_BG, center_cols=[0, 1, 2, 3, 4, 5, 6],
            ))
        story.append(SP(10))

        # Table 2: Keeper cluster
        story.append(P("ClickHouse Keeper Cluster — 3-Node Raft Quorum", S_H2))
        kp_rows = []
        for node in ch_kp.get("nodes", []):
            lbl = node.get("label", "Keeper")
            ...
            kp_rows.append([
                ...
                str(node["vcpu_per_node"]),
                str(node["ram_per_node"]),
                str(ch_kp.get("total_ram_gb",  0)),
                str(ch_kp.get("total_vcpu",    0)),
                f"{node['storage_per_node_gb']} GB",
            ])
        if kp_rows:
            story.append(data_table(
                ["Type", "Node", "vCPU", "RAM (GB)",
                 "Total RAM (GB)", "Total Cores", "Storage/Node"],
                kp_rows,
                col_widths=[2.4*cm, 2.8*cm, 1.6*cm, 2.2*cm, 2.8*cm, 2.4*cm, 2.6*cm],
                accent=ORANGE, alt=ORANGE_BG2, center_cols=[0, 1, 2, 3, 4, 5, 6],
            ))
        story.append(PageBreak())

    # ══════════════════════════════════════════════════════════════════════
    # SECTION 2c — AI SERVICES (when enabled)
    # ══════════════════════════════════════════════════════════════════════
    if ai_enabled:
        AI_PURPLE  = colors.HexColor("#533483")
        AI_BLUE    = colors.HexColor("#0F3460")
        AI_RED     = colors.HexColor("#E94560")
        AI_BG      = colors.HexColor("#F0EEF8")
        AI_BG2     = colors.HexColor("#EEF2FF")

        story += section_header(
            "AI Services Infrastructure",
            "Predictive AI · GenAI · Agentic AI — Kubernetes cluster sizing (Maybank template)",
            color=AI_PURPLE,
        )
        story.append(SP(6))
        ...
        environments = _ai.get("environments", {})
        _type_labels = {"predictive": "Predictive AI", "genai": "GenAI", "agentic": "Agentic AI"}
        _type_colors = {"predictive": AI_PURPLE, "genai": AI_RED, "agentic": AI_BLUE}

        # Summary KPIs
        story.append(info_card([
            ("AI Types Enabled",
             "  ·  ".join(_type_labels[t] for t in environments)),
            ("Total AI Worker Nodes",  str(cs.get("total_worker_nodes", 0))),
            ("Total AI vCPU",          str(cs.get("total_vcpu", 0))),
            ("Total AI RAM",           f"{cs.get('total_ram_gb', 0)} GB"),
            ("Total AI Storage",       f"{cs.get('total_storage_gb', 0):,} GB"),
            ("AWS Bedrock (LLM API)",  f"${bedrock_mo:,.0f}/month  —  No self-hosted GPU"),
        ], fill=AI_BG, accent=AI_PURPLE))
        story.append(SP(10))

        # Per AI-type cluster table
        ai_rows = []
        HOURS_PER_MONTH = 730
        NODE_HOURLY_AVG = 0.576  # blended average
        for ai_type, envs_data in environments.items():
            lbl   = _type_labels.get(ai_type, ai_type.upper())
            prod  = envs_data.get("prod", {})
            if not prod:
                continue
            cl    = prod.get("cluster", {})
            nodes = cl.get("worker_nodes", 0)
            cores = cl.get("total_cores", 0)
            ram   = cl.get("total_ram_gb", 0)
            stor  = cl.get("total_storage_gb", 0)
            dbs   = prod.get("databases", [])
            db_str = "; ".join(
                f"{d['name']} ({d['storage_gb']} GB)" for d in dbs if d.get("storage_gb")
            ) or "—"
            ai_rows.append([
                lbl,
                str(nodes),
                ...
                f"{stor:,} GB",
                db_str,
                f"${round(nodes * NODE_HOURLY_AVG * HOURS_PER_MONTH, 0):,.0f}/mo",
            ])

        if ai_rows:
            story.append(P("AI Cluster Configuration (Production)", S_H2))
            story.append(data_table(
                ["AI Type", "Nodes", "Total Cores", "Total RAM", "Total SSD", "Databases", "Est. Compute/mo"],
                ai_rows,
                col_widths=[2.6*cm, 1.4*cm, 2.0*cm, 2.2*cm, 2.2*cm, 4.0*cm, 2.4*cm],
                accent=AI_PURPLE, alt=AI_BG2, center_cols=[1, 2, 3, 4, 6],
                money_cols=[6],
            ))
            story.append(SP(8))
        ...
        # Bedrock note
        story.append(info_card([
            ("GPU Requirement",  "None — all LLM inference via AWS Bedrock (managed)"),
            ("Bedrock API Cost", f"${bedrock_mo:,.0f}/month  (user-configured, token-based)"),
            ("Annual Bedrock",   f"${bedrock_mo*12:,.0f}"),
            ("5-Year Bedrock",   f"${sum(bedrock_mo*12*(1.04**y) for y in range(1,6)):,.0f}  (at 4% inflation)"),
        ], fill=AI_BG2, accent=AI_BLUE))
        ...
        if priced_roles:
            ...
            for r in priced_roles:
                ...
                rows.append([
                    ...
                    fmt(round(r.get("monthly_usd", 0) * 12, 2)),   # annual = monthly × 12
                ])
            if rows:
                story.append(data_table(
    ...
    if client_mode == "saas" and pricing:
        ...
        for y in range(1, 6):
            ...
            total_usage = prod_dc + prod_dr + preprod + round(ai_mo * mult, 2) + AIRTEL + SOC_MACH + REQ4 + REQ6
