...
def render_node_distribution(distribution: dict):
    ...
    ch_nodes = summary.get("total_clickhouse_nodes", 0)
    ...
    if ch_nodes:
        _metric(s3, "ClickHouse Nodes", ch_nodes)
        _metric(s4, "LLM Used",    "✅ Yes" if summary.get("llm_used") else "📋 Rules only")
        _metric(s5, "Confidence",   summary.get("confidence", "—").capitalize())
    else:
    ...
    # Combine all roles into one table (worker + DB + fixed, ClickHouse shown separately below)
    all_roles = (
    ...
    # Show ClickHouse cluster details if enabled
    ch_sizing = distribution.get("clickhouse_nodes")
    if ch_sizing and ch_sizing.get("enabled"):
        render_clickhouse_sizing(ch_sizing)


# ── ClickHouse Cluster Sizing Display ────────────────────────────────────

def render_clickhouse_sizing(ch_sizing: dict):
    """
    Render ClickHouse cluster sizing in two tables matching the reference image:
      Table 1: ClickHouse DB Cluster (Shards × Replicas)
      Table 2: ClickHouse Keeper Cluster (3 nodes)
    """
    if not ch_sizing or not ch_sizing.get("enabled"):
        return

    st.markdown("")
    st.markdown("### 📊 ClickHouse OLAP Cluster Sizing")

    db_cluster     = ch_sizing.get("db_cluster", {})
    keeper_cluster = ch_sizing.get("keeper_cluster", {})
    summary        = ch_sizing.get("summary", {})

    # ── Summary KPI bar ─────────────────────────────────────────────────────
    k1, k2, k3, k4, k5 = st.columns(5)
    _metric(k1, "Total Nodes",       summary.get("total_nodes", 0))
    _metric(k2, "Shards",            summary.get("num_shards", 0))
    _metric(k3, "Total vCPU",        summary.get("total_vcpu", 0))
    _metric(k4, "Total RAM (GB)",    summary.get("total_ram_gb", 0))
    _metric(k5, "Total Storage (GB)",f"{summary.get('total_storage_gb', 0):,}")

    ch_data_gb = ch_sizing.get("ch_data_gb", 0)
    vol_factor = ch_sizing.get("volume_factor", 1.0)
    st.caption(
        f"📊 Analytics data volume: **{ch_data_gb:,.0f} GB** "
        f"({ch_sizing.get('ch_data_multiplier', 2.0)}× transactional DB × volume factor {vol_factor}×) · "
        f"Self-hosted on EC2 (same for SaaS and On-Prem)"
    )

    # ── Table 1: ClickHouse DB Cluster ──────────────────────────────────────
    st.markdown("**ClickHouse DB Cluster** — Sharded + Replicated (self-hosted)")
    db_rows = []
    for node in db_cluster.get("nodes", []):
        db_rows.append({
            "Shard":          f"Shard {node['shard']}",
            "Node":           f"Node {node['replica']}",
            "CPU (vCPU)": node["vcpu_per_node"],
            "Memory (GB)": node["ram_per_node"],
            "Total Memory\n(this shard)": db_cluster["total_ram_gb"] // db_cluster["num_shards"],
            "Total Cores\n(this shard)": db_cluster["total_vcpu"] // db_cluster["num_shards"],
            "SSD Storage (GB)": f"{node['storage_per_node_gb']:,}",
        })
    if db_rows:
        st.dataframe(pd.DataFrame(db_rows), hide_index=True, use_container_width=True)

    # ── Table 2: ClickHouse Keeper Cluster ─────────────────────────────────
    st.markdown("**ClickHouse Keeper Cluster** — 3-node Raft quorum (coordination)")
    keeper_rows = []
    for node in keeper_cluster.get("nodes", []):
        keeper_rows.append({
            "Node Type":      "Keeper Node",
            "Node":           node["label"].split("—")[-1].strip() if "—" in node["label"] else node["label"],
            "CPU (vCPU)": node["vcpu_per_node"],
            "Memory (GB)": node["ram_per_node"],
            "Total Memory\n(cluster)": keeper_cluster["total_ram_gb"],
            "Total Cores\n(cluster)": keeper_cluster["total_vcpu"],
            "SSD Storage (GB)": f"{node['storage_per_node_gb']:,}",
        })
    if keeper_rows:
        st.dataframe(pd.DataFrame(keeper_rows), hide_index=True, use_container_width=True)
...
def render_pricing_results(pricing: dict, updated_file: str = None):
    ...
    dl2.download_button(
        ...
        file_name="cloud_pricing.json", mime="application/json",
...
def render_ai_sizing(ai_sizing: dict):
    """
    Render AI Services infrastructure sizing in the Streamlit results area.
    Shows KPI summary bar + per-AI-type tables for each environment.
    """
    if not ai_sizing or not ai_sizing.get("enabled"):
        return

    st.markdown("---")
    st.markdown("## 🤖 AI Services Sizing")
    ...
    # KPI bar
    k1, k2, k3, k4, k5 = st.columns(5)
    _metric(k1, "Total AI Nodes",    cs.get("total_worker_nodes", 0))
    _metric(k2, "Total vCPU",        cs.get("total_vcpu", 0))
    _metric(k3, "Total RAM (GB)",    cs.get("total_ram_gb", 0))
    _metric(k4, "Total Storage (GB)", f"{cs.get('total_storage_gb', 0):,}")
    _metric(k5, "Bedrock/mo",        f"${bedrock:,.0f}")

    st.caption(
        "📊 Sizing derived from Maybank Hardware Sizing AI Template.  "
        "AWS Bedrock provides managed LLM inference — no self-hosted GPU required."
    )

    environments = ai_sizing.get("environments", {})
    type_labels  = {"predictive": "Predictive AI", "genai": "GenAI", "agentic": "Agentic AI"}

    for ai_type, envs_data in environments.items():
        icon  = _AI_TYPE_ICONS.get(ai_type, "🔷")
        label = type_labels.get(ai_type, ai_type.upper())
        color = _AI_TYPE_COLORS.get(ai_type, "#333")

        st.markdown(
            f"<div style='background:{color};padding:8px 16px;border-radius:8px;"
            f"color:#fff;font-weight:700;font-size:14px;margin:12px 0 6px 0;'>"
            f"{icon} {label}</div>",
            unsafe_allow_html=True,
        )
        ...
        for env_key, env_data in envs_data.items():
            ...
            with st.expander(f"📂 {env_label}", expanded=(env_key == "prod")):
                # Cluster KPIs
                ck1, ck2, ck3, ck4 = st.columns(4)
                _metric(ck1, "Worker Nodes",  cluster.get("worker_nodes", 0))
                _metric(ck2, "Total Cores",   cluster.get("total_cores", 0))
                _metric(ck3, "Total RAM (GB)", cluster.get("total_ram_gb", 0))
                _metric(ck4, "Total SSD (GB)", f"{cluster.get('total_storage_gb',0):,}")

                # Services table
                if services:
                    st.markdown("**DataNext Services — Pod Requirements**")
                    import pandas as pd
                    svc_rows = []
                    for s in services:
                        svc_rows.append({
                            ...
                            "CPU/Pod":       s["cpu_per_pod"],
                            "RAM/Pod (GB)":  s["ram_per_pod"],
                            "Pods":          s["pods"],
                            "Total CPU":     s["cpu_per_pod"] * s["pods"],
                            "Total RAM (GB)":s["ram_per_pod"] * s["pods"],
                            "Storage (GB)":  s.get("storage_gb", 0) or 0,
                        })
                ...
                if databases:
                    ...
                    for db in databases:
                        db_rows.append({
                            "Database":      db["name"],
                            "Type":          db.get("type", "—"),
                            "Pods":          db.get("pods", "—") or "N/A",
                            "Total CPU":     db.get("total_cpu", "—") or "N/A",
                            "Total RAM (GB)":db.get("total_ram_gb", "—") or "N/A",
                            "Storage (GB)":  db.get("storage_gb", 0),
                            "Persistent":    "✅" if db.get("persistent") else "—",
                        })
                    st.dataframe(pd.DataFrame(db_rows), hide_index=True,
                                 use_container_width=True)

                st.caption(f"⚡ GPU: AWS Bedrock (Managed) — no self-hosted GPU hardware.")
