...
metrics extracted from the sizing template:
  ...
  2. DR                    — compute scaled by user-selected DR scale (50% or 100%)
                             DB always replicated at 100% of Production
...
EC2_FALLBACK = {
    "t3.medium":   0.0416,
    "t3a.medium":  0.0376,   # AMD EPYC — always used for Bastion host
...
def _build_env_roles(
    ...
    dr_compute_scale: float = None,    # if set: overrides scale for COMPUTE only; DB stays full prod
) -> list:
    """
    Build a list of priced roles for one environment (Pre-Prod or DR),
    scaling from actual Production metrics.

    scale=0.40            → Pre-Prod/SIT/UAT (compute + DB both scaled)
    dr_compute_scale=0.5  → DR 50%: compute halved, DB at full prod
    dr_compute_scale=1.0  → DR 100%: compute at full prod, DB at full prod

    deployment="onprem" → EC2 self-hosted for all (SQL Server BYOL on AMD)
    deployment="saas"   → EKS managed K8s, EC2 workers
    """
    mult = AWS_REGIONS.get(region, {}).get("multiplier", 1.0)
    ...
    # ── Scale compute and DB separately ───────────────────────────────────
    if dr_compute_scale is not None:
        # DR mode: compute uses dr_compute_scale, DB always at full production spec
        _c_scale = dr_compute_scale          # for infra/cache/alb node decisions
        # Pilot-light (50% DR): keep exactly 1 worker node running.
        # The intent is minimal footprint — nodes are scaled up only when DR is activated.
        # Full mirror (100% DR): replicate all production worker nodes.
        env_worker_nodes  = 1 if _c_scale < 1.0 else prod_worker_nodes
        # Pilot-light: smaller per-node instance — half prod vCPU/RAM → selects a much cheaper type
        # Full mirror: keep full production per-node specs
        if _c_scale < 1.0:
            env_vcpu_per_node = max(4,  prod_vcpu_per_node // 2)
            env_ram_per_node  = max(8,  prod_ram_per_node  // 2)
        else:
            env_vcpu_per_node = max(4,  prod_vcpu_per_node)
            env_ram_per_node  = max(16, prod_ram_per_node)
        # DB: always full production (never scaled in DR)
        env_db_ram   = prod_db_ram
        env_db_vcpu  = prod_db_vcpu
        env_db_nodes = 2   # DR always uses Multi-AZ
        # Storage: DB SAN stays full prod; S3 scales with compute
        env_data_gb  = data_gb                                        # full prod SAN/DB data
        env_s3_gb    = max(500, math.ceil(s3_gb * _c_scale))
        worker_storage = 128   # pilot-light: smaller EBS per node
    else:
        # Pre-Prod mode: scale compute and DB proportionally
        _c_scale = scale
    ...
    worker_itype, worker_hr, worker_from_api = _resolve_ec2_instance(
        env_vcpu_per_node, env_ram_per_node, "memory", pricing_client, ec2_client, region
    )
    ...
    bastion_itype= "t3a.medium"   # Always AMD t3a.medium (policy: fixed bastion type)
    ...
    priced.append({
        ...
        "storage_per_node_gb": worker_storage,
    ...
    # Scale: 1 node for pre-prod; DR uses _c_scale threshold
    infra_nodes = 2 if _c_scale >= 0.75 else 1
    infra_vcpu, infra_ram = 8, 16
    infra_storage = 512
    infra_itype, infra_hr, infra_from_api = _resolve_ec2_instance(
        infra_vcpu, infra_ram, "memory", pricing_client, ec2_client, region
    )
    ...
    priced.append({
        ...
        "storage_per_node_gb": infra_storage,
    ...
    # ── SAN storage ────────────────────────────────────────────────────────
    san_cost = env_data_gb * env_db_nodes * EBS_GP3_PER_GB * mult
    priced.append({
        "category": db_cat,
        "label":    f"Storage: SAN – {env_label}",
        "nodes": env_db_nodes, "vcpu": 0, "ram": 0,
        "storage_per_node_gb": env_data_gb,
        "instance_type": "EBS gp3",
        "monthly_usd": round(san_cost, 2),
        "note": f"{env_db_nodes}× {env_data_gb}GB gp3 SAN ({('full prod DB' if dr_compute_scale is not None else f'scaled {scale*100:.0f}% of prod')})",
    })
    ...
    priced.append({
        ...
        "note": f"{env_s3_gb}GB S3 (scaled {_c_scale*100:.0f}% of prod {s3_gb}GB)",
    })

    # ── ElastiCache ────────────────────────────────────────────────────────
    cache_nodes = 2 if _c_scale >= 0.75 else 1
    ...
    # ── Load balancer ──────────────────────────────────────────────────────
    alb_nodes = 2 if _c_scale >= 0.75 else 1
    ...
    # ── Backup ────────────────────────────────────────────────────────────
    backup_gb   = max(1000, math.ceil((data_gb + s3_gb) * _c_scale))
    ...
    priced.append({
        ...
        "note": f"{backup_gb}GB S3 backup (scaled {_c_scale*100:.0f}% of prod)",
    })
...
# ── ClickHouse environment pricing ─────────────────────────────────────────

def _price_clickhouse_env(
    ch_prod_sizing: dict,
    env_label:      str,
    env_type:       str,        # 'preprod' | 'dr'
    dr_compute_scale: float = 0.5,
    region:         str     = "us-east-1",
    pricing_client  = None,
    ec2_client      = None,
) -> list:
    """
    Price ClickHouse cluster for Pre-Prod or DR environments.

    Pre-Prod: 1 shard × 1 replica (minimal footprint) + 1 Keeper node
    DR (50%):  same shard count as prod × 1 replica + 3 Keeper nodes (Raft quorum)
    DR (100%): same as production
    """
    if not ch_prod_sizing or not ch_prod_sizing.get("enabled"):
        return []

    mult     = AWS_REGIONS.get(region, {}).get("multiplier", 1.0)
    priced   = []
    prod_db  = ch_prod_sizing.get("db_cluster", {})

    if env_type == "preprod":
        env_shards    = 1
        env_replicas  = 1
        env_vcpu      = max(8, prod_db.get("vcpu_per_node", 32) // 4)
        env_ram       = env_vcpu * 4
        env_storage   = max(500, math.ceil(prod_db.get("storage_per_node_gb", 2000) * 0.40))
        keeper_count  = 1
    else:
        env_shards    = prod_db.get("num_shards", 2)
        env_replicas  = 1 if dr_compute_scale < 1.0 else prod_db.get("replicas_per_shard", 2)
        env_vcpu      = prod_db.get("vcpu_per_node", 32) if dr_compute_scale >= 1.0 else max(8, prod_db.get("vcpu_per_node", 32) // 2)
        env_ram       = env_vcpu * 4
        env_storage   = prod_db.get("storage_per_node_gb", 2000)
        keeper_count  = 3

    env_db_nodes = env_shards * env_replicas

    try:
        db_itype, db_hr, db_from_api = _resolve_ec2_instance(
            env_vcpu, env_ram, "memory", pricing_client, ec2_client, region
        )
    except Exception:
        db_itype, db_hr, db_from_api = "r5.8xlarge", 2.016, False

    db_compute = db_hr * env_db_nodes * HOURS_PER_MONTH
    db_storage = env_storage * env_db_nodes * EBS_GP3_PER_GB * mult
    priced.append({
        "category":     "ClickHouse DB Cluster",
        "label":        f"ClickHouse DB Nodes ({env_shards}s×{env_replicas}r) – {env_label}",
        "nodes":        env_db_nodes,
        "vcpu":         env_vcpu,
        "ram":          env_ram,
        "instance_type": db_itype,
        "hourly_usd":   round(db_hr, 4),
        "monthly_usd":  round(db_compute + db_storage, 2),
        "note": (
            f"{env_db_nodes}× {db_itype} + {env_storage:,}GB gp3 — "
            f"{env_shards} shard(s) × {env_replicas} replica(s) [{env_type}]"
        ),
        "from_api":     db_from_api,
    })

    try:
        k_itype, k_hr, k_from_api = _resolve_ec2_instance(
            4, 16, "general", pricing_client, ec2_client, region
        )
    except Exception:
        k_itype, k_hr, k_from_api = "m5.xlarge", 0.192, False

    k_compute = k_hr * keeper_count * HOURS_PER_MONTH
    k_storage = 200 * keeper_count * EBS_GP3_PER_GB * mult
    priced.append({
        "category":     "ClickHouse Keeper Cluster",
        "label":        f"ClickHouse Keeper Nodes ({keeper_count}) – {env_label}",
        "nodes":        keeper_count,
        "vcpu":         4,
        "ram":          16,
        "instance_type": k_itype,
        "hourly_usd":   round(k_hr, 4),
        "monthly_usd":  round(k_compute + k_storage, 2),
        "note":         f"{keeper_count}× {k_itype} + 200GB gp3 [{env_type}]",
        "from_api":     k_from_api,
    })
...
def price_additional_environments(
    ...
    dr_scale:       float = 1.0,   # 0.5 = 50% compute, DB full prod; 1.0 = full mirror
    distribution:   dict  = None,  # production node distribution (for ClickHouse)
) -> dict:
    """
    ...
    # Resolve production ClickHouse sizing (if enabled)
    ch_prod_sizing = None
    if distribution:
        ch_prod_sizing = distribution.get("clickhouse_nodes")
    ...
    # Inject ClickHouse for Pre-Prod (1 shard × 1 replica + 1 Keeper)
    if ch_prod_sizing and ch_prod_sizing.get("enabled"):
        ch_preprod = _price_clickhouse_env(
            ch_prod_sizing=ch_prod_sizing,
            env_label="Pre-Prod",
            env_type="preprod",
            region=preprod_region,
            pricing_client=pricing_client,
        ...
        preprod_roles.extend(ch_preprod)
    # SIT + UAT = two separate environments, each sized at PREPROD_SCALE.
    # The roles/costs above are priced for ONE environment (the "lower env").
    # Multiply by 2 to get the combined SIT + UAT total.
    preprod_cats, preprod_total = _summarise(preprod_roles)
    sit_uat_monthly = round(preprod_total * 2, 2)
    sit_uat_annual  = round(sit_uat_monthly * 12, 2)

    preprod_result = {
        "priced_roles":    preprod_roles,      # roles for 1 env (used for itemised Excel rows)
        "category_totals": preprod_cats,
        "per_env_monthly": preprod_total,       # single SIT or UAT env cost (for reference)
        "monthly_usd":     sit_uat_monthly,     # SIT + UAT combined = 2× single env
        "annual_usd":      sit_uat_annual,
    }
    ...
    dr_roles = _build_env_roles(
        ...
        db_type=db_type,
        single_az=False,
        dr_compute_scale=dr_scale,
    )
    # Inject ClickHouse for DR
    if ch_prod_sizing and ch_prod_sizing.get("enabled"):
        ch_dr = _price_clickhouse_env(
            ch_prod_sizing=ch_prod_sizing,
            env_label="DR",
            env_type="dr",
            dr_compute_scale=dr_scale,
    ...
    return {
        ...
        "is_saas":          is_saas,
        "dr_scale":         dr_scale,
        "clickhouse_enabled": bool(ch_prod_sizing and ch_prod_sizing.get("enabled")),
    }
