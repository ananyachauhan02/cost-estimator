...
| SAN storage (Primary) | 2 | `data_size_gb` GB per node (gp3) |
| S3 storage | 1 | `s3_size_gb` GB |
| Reporting DB (optional) | 1 | 32 vCPU / 128 GB / 300 GB EBS |
| Reporting SAN (optional) | 1 | `data_size_gb` GB (gp3) |
...
### 4.3 SAN / Database Storage Cost (gp3)

```
monthly_usd = storage_gb_per_node × node_count × EBS_GP3_PER_GB × region_multiplier
```
...
san_cost    = env_data_gb × env_db_nodes × EBS_GP3 × region_mult
